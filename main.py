...
def main():
    ...
    menu_status = None  # (Đã loại bỏ thông báo, không sử dụng menu_status nữa)
    ...
    while True:
                ...
                if evt_type == "server_message":
                    ...
                    if ptype == "lobby":
                        ...
                        if "room_id" in payload:
                            current_room_id = payload["room_id"]
                    elif ptype == "state":
        ...
        # Hiển thị room_id ở góc phải trên nếu có
        # Luôn dùng biến current_room_id để hiển thị góc phải
        room_id = current_room_id if game_mode in {"network_host", "network_client"} else None
        from src.ui.pygame_renderer import draw_room_id
        draw_room_id(screen, room_id)
